...
def train(option="rft"):
    ...
    for school_id in tqdm(range(len(schoolname.keys()))):
        ...
        if len(df_test[df_test["受験校"] == schoolname[school_id]]) > 0:
            ...
            test = df_train[[df_train[df_train[df_train["年度"]==2020]]["受験校"] == schoolname[school_id]][paras_all]]
            df_original = df_train[[df_train[df_train[df_train["年度"]!=2020]]["受験校"] == schoolname[school_id]][paras_all]]
            if len(test[test["受験校"] == schoolname[school_id]]) > 0:
                pass
            else:
                remove_list.append(schoolname[school_id])
