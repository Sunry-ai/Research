...
def train(option="rft",train_mode="each_school"):
    ...
    if train_mode=="each_school":
        ...
        for df_school in df_list:
            X_train, X_test, y_train, y_test = train_test_split(df_school[paras], df_school[y_para],test_size=0.3, random_state=42)
        # ------------------------------------------------------训练&评价--------------------------------------------------------------------------------
            if option=="rft":
                try:
                    ...
                    rft_mode(X_train[paras], y_train[y_para], str(school_id))
                    with open(model_save + "/" + str(school_id) + ".joblib", mode="rb") as fp:
                        clf = pickle.load(fp)
                ...
                try:
                    ...
                    lgb_model(X_train[paras], y_train[y_para],X_test[paras],y_test[y_para], str(school_id))
                    with open(model_save + "/" + str(school_id) + ".joblib", mode="rb") as fp:
                        clf = pickle.load(fp)
