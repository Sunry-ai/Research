...
def train(option="rft"):
    ...
    for school_id in tqdm(range(len(schoolname.keys()))):
        ...
        # df_original为单个学校的数据
        try:
            df_original = df_train.groupby("受験校").get_group(schoolname[school_id])[
                paras_all
            ]
        except:
            print("error")
            df_original = df_test.groupby("受験校").get_group(schoolname[school_id])[
                paras_all
            ]
        # 如果测试数据集中没有该学校则使用原本数据集
        print("これ",df_test[df_test["受験校"]])
        if len(df_test[df_test["受験校"] == schoolname[school_id]]) > 0:
            ...
            test = df_train[df_train["受験校"] == schoolname[school_id]][paras_all]
...
if __name__ == "__main__":
    ...
    paras, schoolname, df_ = feature_maker.main()
    print("年度 in traing",df_["年度"].unique())
    # Params設定
