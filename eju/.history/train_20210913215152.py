...
def train(option="rft",train_mode="each_school"):
    ...
    if train_mode=="whole":
        ...
        if option=="rft":
            try:
                with open(model_save + "/" + "rft" + ".joblib", mode="rb") as fp:
                    clf = pickle.load(fp)
            except:
                rft_mode(X_train[paras], y_train[y_para], "rft")
                with open(model_save + "/" + "rft" + ".joblib", mode="rb") as fp:
                    clf = pickle.load(fp)
            ...
            try:
                ...
                lgb_model(X_train[paras], y_train[y_para],X_test[paras],y_test[y_para], "lgb")
                with open(model_save + "/" + "lgb" + ".joblib", mode="rb") as fp:
                    clf = pickle.load(fp)
        ...
        mlflow.log_metric("precistion", precistion)
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="训练与测试数据集"):
        ...
        train(option="lgb",train_mode="whole")
