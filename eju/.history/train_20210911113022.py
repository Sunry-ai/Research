...
def train(option="rft"):
    ...
    if train_mode="each_school":
        for df_school in df_list:
            df_train=df_school[df_school.年度!="2021"]
            X_train, X_test, y_train, y_test = train_test_split(df_train[paras], df_train[y_para],test_size=0.3, random_state=42)

            df_test=df_school[df_school.年度=="2021"]
        # ------------------------------------------------------训练&评价--------------------------------------------------------------------------------
            if option=="rft":
                try:
                    ...
                    rft_mode(df_train[paras], df_train[y_para], str(school_id))
                    with open(model_save + "/" + str(school_id) + ".joblib", mode="rb") as fp:
                        clf = pickle.load(fp)
                ...
                try:
                    ...
                    lgb_model(df_train[paras], df_train[y_para],df_test[paras],df_test[y_para], str(school_id))
                    with open(model_save + "/" + str(school_id) + ".joblib", mode="rb") as fp:
                        clf = pickle.load(fp)
    ...
    #3)评价
        ...
        for item in test_model:
            test_predicted.append(item)
        for item in real:
            test_real.append(item)
    ...
    result_model = [final_result(item, 0.5) for item in test_predicted]
    print("len is",len(test_real),len(result_model))
    for i in range(len(test_real)):
        if test_real[i] == result_model[i]:
            count += 1
        else:
            pass

    accuracy = count / len(test_real)

    recall = recall_score(result_model, test_real)
    precistion = precision_score(result_model, test_real)
    print(accuracy,recall,precistion)
